...
class KBinsDiscretizer:
    ...
    def encode(self, X, subslice=None, indices=None):
        if X.ndim == 1:
            X = X[None]

        assert subslice is None or indices is None, 'subslice and indices should not be given at the same time'
        if subslice is None and indices is None:
            bin_edges = self.bin_edges
        elif subslice is not None:
            start, end = subslice
            bin_edges = self.bin_edges[start:end]
        else:
            bin_edges = np.stack([self.bin_edges[i] for i in indices])
    ...
    def decode(self, Xt, subslice=None):
        ...
        if isinstance(Xt, torch.Tensor):
            bin_centers = self.bin_centers_torch
            X = torch.zeros_like(Xt, dtype=torch.float)
            map_int = lambda x: x
        else:
        ...
        for jj in range(Xt.shape[1]):
            X[:, jj] = bin_centers[jj, map_int(Xt[:, jj])]
